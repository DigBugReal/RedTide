...
RecalculateStats.add(function(actor, api)
	...
	api.pHmax_add(0.7) --note to self on movespeed calc: desired % * 0.028 (25 * 0.028 = 0.7)
...
Callback.add(Callback.ON_SKILL_ACTIVATE, function(actor, slot)
    ...
    actor:buff_apply(undyBuff, 60 * (5 * stack))
	local x, y, team = actor.x, actor.y, actor.team
    
	local circ = Object.find("EfCircle", "ror")
	local boom = circ:create(actor.x, actor.y)
	boom.radius = 250
	boom.image_blend = Color.YELLOW
	
    -- Apply to all nearby allies
    local actors = Instance.find_all(gm.constants.pActor)
    for _, a in ipairs(actors) do
        if  a.team == team
        and math.distance(a.x, a.y, x, y) <= 250 then
            a:buff_apply(undyBuff, 60 * (5 * stack))
        end
    end
end)